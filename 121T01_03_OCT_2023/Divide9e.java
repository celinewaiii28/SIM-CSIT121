...
//                 ==> n is divisible by 9
// File name: Divide9e.java
// Explore some user defined method to handle for some of the sub tasks
// How to access to non static method define inside the class
...
class Divide9e
{
	...
	private boolean testDivisible (int sum)
	{
		if (sum % 9 == 0)
			return true;
		else
	...
	{
		// Construct an object of this class (Divide9e)
		Divide9e d9e = new Divide9e ();
		...
		for (int i = 1; i <= 6; i++)
		{
			...
			int sum = d9e.getSumOfDigit (n);
			
			//Test divisible by 9
			boolean ok = d9e.testDivisible (sum);
			if (ok )
